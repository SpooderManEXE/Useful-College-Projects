...
class Paddle:
    ...
    def collides_with_ball(self, ball):
        left_side_hit, left_side_overlap = self.__collides_with_ball(
            ball, self.x)
        if left_side_hit:
            return left_side_hit, left_side_overlap

        right_side_hit, right_side_overlap = self.__collides_with_ball(
            ball, self.x + self.width)
...
def ai_move(ball, computer):
    if not round_winner and ball.direction_x == 1 and ball.x > SCREEN_WIDTH // 2:
        ...
        if predicted_y > 0 and predicted_y < SCREEN_HEIGHT and (predicted_y < computer.y or predicted_y > computer.y + computer.height):
            computer.y += COMPUTER_MOVEMENT_UNITS * \
                (-1 if predicted_y < computer.y else 1)
...
while True:
    ...
    computer_goal_reached, computer_defended = computer.collides_with_ball(
        ball)
    ...
    pygame.draw.rect(screen, player.color, (player.x,
                                            player.y, player.width, player.height), 0)
    pygame.draw.rect(screen, computer.color, (computer.x,
                                              computer.y, computer.width, computer.height), 0)
    pygame.draw.circle(screen, ball.color, (ball.x, ball.y), ball.radius)
    for i in range(1, SCREEN_HEIGHT // 10):
        if i % 2 == 0:
            pygame.draw.rect(
                screen, GRAY, (SCREEN_WIDTH // 2, i * 10, 10, 10), 0)
    ...
    if keys[pygame.K_UP]:
        ...
        if goal_reached and player.y - PLAYER_MOVEMENT_UNITS < ball.y + ball.radius:
            continue
        player.y = 0 if player.y - \
            PLAYER_MOVEMENT_UNITS < 0 else player.y - PLAYER_MOVEMENT_UNITS
        last_key = -1
    ...
    if keys[pygame.K_DOWN]:
        ...
        if goal_reached and player.y + player.height + PLAYER_MOVEMENT_UNITS > ball.y - ball.radius:
            continue
        player.y = SCREEN_HEIGHT - player.height if player.y + player.height + \
            PLAYER_MOVEMENT_UNITS > SCREEN_HEIGHT else player.y + PLAYER_MOVEMENT_UNITS
        last_key = 1
